...
setwd("~/Documents/IMMRSE/Hrp2:3/Scripts/P.falciparum_HRP2_3_project")
...
ddpcr <- read_csv("../../data_out/All_ddPCR_raw_data_26032024.csv")
...
length(unique(ddpcr_filtered$`Sample Name`)) # 108 samples
...
any(is.na(ddpcr_with_parasite_density_and_controls$`Study Subject`)) # Missing study subject id's
...
any(is.na(ddpcr_with_parasite_density_and_controls_merged$`Study Subject`))# TRUE missing id's. #

na <- filter(ddpcr_with_parasite_density_and_controls_merged, is.na(`Study Subject`))
...
# Group
ddpcr_sub<- ddpcr_with_parasite_density_and_controls_merged %>% 
  # group_by(`Sample Name`, Target) %>% 
  #mutate(Replicate = row_number(`Sample Name`))  %>% 
  select(-Well, -Valid_Partitions)

## Take look at control data # Need to evaluate how we analyse the controls. Merging them across runs
# causing several replicates; perhaps evaluate control data per run separately?

ddpcr_controls <- ddpcr_sub[grepl(ddpcr_sub$`Study Subject`, pattern= "Dd2|3D7", ignore.case = TRUE),  ]
...
## Filter out IMMRSE samples, controls and sample that was not part of the LLNEUP 12 samples but ran accidentally


ddpcr_sub2 <-ddpcr_sub[!grepl(ddpcr_sub$`Study Subject`, pattern = "^IM|^W780|Dd2|3D7"), ] 


# Pivot wide and filter out the controls and study subject with missing id
ddpcr_wide<- ddpcr_sub2 %>%  
  filter(!is.na(`Study Subject`)) %>% 
  group_by(`Sample Name`, Target) %>% 
  mutate(Replicate = row_number(`Sample Name`)) %>% 
  mutate(Target_rep = paste0(Target,"_rep", Replicate)) %>% 
  pivot_wider(id_cols = c(`Sample Name`, `Study Subject`, qpcr), names_from = Target_rep, values_from = c("Concentration", "Positive_partitions") ) 
