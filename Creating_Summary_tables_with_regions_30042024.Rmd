...
### Calculating z statistics to compare observed proportions between groups
```{r}
Bound_tables <- rbind(Summary_table, Summary_table_2)

Transposed <- t(Bound_tables)

Transposed_2 <- as.data.frame(Transposed)

Transposed_2$category <-row.names(Transposed_2)

Transposed_2_filtered <- Transposed_2[!grepl(pattern ="*Prop*", Transposed_2$category), ]

Transposed_2_filtered <- Transposed_2_filtered %>% 
  transmute(category, Concordant, Discordant)


z_test_stats_fever <- prop.test(x= c( nrow(Fever_status_concordant_12_24), nrow(Fever_status_discordant_12_24)), n =c(nrow(Characteristics_concordant_12_24), nrow(Characteristics_discordant_12_24)))

z_test_stats_fever

# Is the observed proportion of those with fever among concordant samples greater than that in the discordant group?
z_test_stats_fever_greater <- prop.test(x= c( nrow(Fever_status_concordant_12_24), nrow(Fever_status_discordant_12_24)), n =c(nrow(Characteristics_concordant_12_24), nrow(Characteristics_discordant_12_24)), alternative = "greater")

z_test_stats_fever_greater
```
...
```{r}
wilcox.test(Characteristics_concordant_12_24$parasitedensity, Characteristics_discordant_12_24_merged$parasitedensity, alternative = "greater", conf.int = TRUE)
```
