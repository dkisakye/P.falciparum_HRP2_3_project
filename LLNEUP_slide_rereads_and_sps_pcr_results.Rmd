---
title: "Summarising_sps_rereads and Species pcr data"
author: "dkisakye"
date: "2023-09-18"
output:
  html_document:
...
### Import set 1 of reread slides
```{r}
microscopy_rereads_set1_rep1 <- read_xlsx("../../slide_re_reads/LLNEUP_Microscopy_read_results.xlsx", sheet = "LLN2-12months_BS_First_Read") %>% 
...
all_sps_results <- all_sps_results %>% 
  rename(Overall_call_slide_reread = QC)


# Get no. of distinct ids
distinct_ids <-distinct(all_sps_results, Barcode)

length(distinct_ids$Barcode) # 75
...
# merge with var_ATS results

var_ATS_data <- read_csv("../../data_out/var_ATS_neg_RDT_Neg_RDT_Pos_LLN2_04032023.csv")

var_ATS_data <- var_ATS_data %>% 
  transmute(rdtrslt, BSdich,  parasitedensity_microscopy = parasitedensity, Quantity_by_qpcr = Quantity,
            Barcode, fpbarcode1)

all_sps_results_merge <- var_ATS_data %>% 
  left_join(all_sps_results, join_by(fpbarcode1 == `Barcode`))


all_sps_results_merge$Barcode <- as.character(all_sps_results_merge$Barcode)
```
...
merged_sps_pcr_slide_rereads <- sps_pcr_calls_2 %>% 
  left_join(all_sps_results_merge, join_by(Barcode))
...
# let me look briefly at the discordant samples between re-reads and pcr
