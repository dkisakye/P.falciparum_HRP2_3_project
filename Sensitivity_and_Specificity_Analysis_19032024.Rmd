...
sensitivity_by_bs <-round(sensitivity(Household_data_trim$rdtrslt, Household_data_trim$BSdich, positive = levels(Household_data_trim$BSdich)[1]) * 100, 0) # 100
...
pos_Pred_bs <- round(posPredValue(Household_data_trim$rdtrslt, Household_data_trim$BSdich, positive = levels(Household_data_trim$BSdich)[1])*100, 0) # 50

pos_Pred_bs

#specificity by BS, all LN2 samples
spec_by_bs <- round(specificity( Household_data_trim$rdtrslt, Household_data_trim$BSdich, negative = levels(Household_data_trim$BSdich)[-1])* 100, 0) # 56
...
kable(results, format = "html", col.names = c("Measure", "Value"), 
      caption = "Performance Metrics Including Confusion Matrix") %>%
...
## Could be interesting to look at sensitivity/specificity of RDTs by parasite density (qPCR)
